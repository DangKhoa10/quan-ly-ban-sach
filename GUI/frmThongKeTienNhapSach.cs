...
{
    public partial class frmThongKeTienNhapSach : Form
    {
        public frmThongKeTienNhapSach()
        {
            InitializeComponent();
        }
        clsHonLoan honLoan;
        private void frmThongKeTienNhapSach_Load(object sender, EventArgs e)
        {
        ...
        {
            cboTenNhanVien.DataSource = new clsNhanVien().GetNVTKCSN();
        ...
        void NameColumn()
        {
            dgvDSHoaDon.Columns[0].HeaderText = "Mã phiếu nhập";
            dgvDSHoaDon.Columns[0].Width = 180;
            dgvDSHoaDon.Columns[1].HeaderText = "Ngày lập phiếu";
            dgvDSHoaDon.Columns[1].Width = 180;
            dgvDSHoaDon.Columns[1].DefaultCellStyle.Format = "dd/MM/yyyy";
            dgvDSHoaDon.Columns[2].HeaderText = "Nhân viên lập phiếu";
            dgvDSHoaDon.Columns[2].Width = 250;
            dgvDSHoaDon.Columns[3].HeaderText = "Tổng giá trị phiếu nhập (VNĐ)";
        ...
        private void btnThongKe_Click(object sender, EventArgs e)
        {
            ...
            if (rdoTheoKhoangThoiGian.Checked)
            {
                ...
                else if (chkNhanVien.Checked)
                {
                    ...
                    {
                        dgvDSHoaDon.DataSource = honLoan.ThongKeChiNhapSachTheoNhanVien(dtmTuNgay.Value, dtmDenNgay.Value, cboTenNhanVien.SelectedValue.ToString());
                        NameColumn();
                        tinhTongTien(honLoan.ThongKeChiNhapSachTheoNhanVien(dtmTuNgay.Value, dtmDenNgay.Value, cboTenNhanVien.SelectedValue.ToString()).ToList());
                    }
                }
                else
                {
                    dgvDSHoaDon.DataSource = honLoan.ThongKeChiNhapSach(dtmTuNgay.Value, dtmDenNgay.Value);
                    NameColumn();
                    tinhTongTien(honLoan.ThongKeChiNhapSach(dtmTuNgay.Value, dtmDenNgay.Value).ToList());
                }
            }
            if (rdoNgay.Checked)
            {
                ...
                else if (chkNhanVien.Checked)
                {
                    ...
                    {
                        dgvDSHoaDon.DataSource = honLoan.ThongKeChiNhapSachTheoNhanVienMotNgay(dtmOneDay.Value, cboTenNhanVien.SelectedValue.ToString());
                        NameColumn();
                        tinhTongTien(honLoan.ThongKeChiNhapSachTheoNhanVienMotNgay(dtmOneDay.Value, cboTenNhanVien.SelectedValue.ToString()).ToList());
                    }
                }
                else
                {
                    dgvDSHoaDon.DataSource = honLoan.ThongKeChiNhapSachMotNgay(dtmOneDay.Value);
                    NameColumn();
                    tinhTongTien(honLoan.ThongKeChiNhapSachMotNgay(dtmOneDay.Value).ToList());
                }
            }
            if (rdoThang.Checked)
            {
                if (mtbThangNam.Text.KiemTraThangNam() && mtbThangNam.Text.EndsWith("0000") == false)
                {
                    ...
                    else if (chkNhanVien.Checked)
                    {
                        ...
                        {
                            dgvDSHoaDon.DataSource = honLoan.ThongKeChiNhapSachTheoNhanVienThang(ngay, cboTenNhanVien.SelectedValue.ToString());
                            NameColumn();
                            tinhTongTien(honLoan.ThongKeChiNhapSachTheoNhanVienThang(ngay, cboTenNhanVien.SelectedValue.ToString()).ToList());
                        }
                    }
                    else
                    {
                        dgvDSHoaDon.DataSource = honLoan.ThongKeChiNhapSachMotThang(ngay);
                        NameColumn();
                        tinhTongTien(honLoan.ThongKeChiNhapSachMotThang(ngay).ToList());
                    }
                }
                else
                {
            ...
            if (rdoNam.Checked)
            {
                ...
                else
                {
                    ...
                    else if (chkNhanVien.Checked)
                    {
                        ...
                        {
                            dgvDSHoaDon.DataSource = honLoan.ThongKeChiNhapSachTheoNhanVienNam(nam, cboTenNhanVien.SelectedValue.ToString());
                            NameColumn();
                            tinhTongTien(honLoan.ThongKeChiNhapSachTheoNhanVienNam(nam, cboTenNhanVien.SelectedValue.ToString()).ToList());
                        }
                    }
                    else
                    {
                        dgvDSHoaDon.DataSource = honLoan.ThongKeChiNhapSachMotNam(nam);
                        NameColumn();
                        tinhTongTien(honLoan.ThongKeChiNhapSachMotNam(nam).ToList());
                    }
                }
            }
        }
        ...
        private void frmThongKeTienNhapSach_FormClosing(object sender, FormClosingEventArgs e)
        {
            DialogResult r;
